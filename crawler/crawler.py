...
def query_missed():
    ...
    for offset in missed:
        ...
        for i in range(0, LAST_NUMS_CONSECUTIVE_USERS, NUMS_CONSECUTIVE_USERS):
            payload = c.get_payload(offset+i)
            print(offset+i, payload)
            try:
                resp = c.query(payload)
            except:
...
def save_to_file(res):
    with open(f'{CODEJAM_ROUND}_{NUMS_CONSECUTIVE_USERS}.json', 'w') as f:
        json.dump(res, f)


save_to_file(query_all())
# save_to_file(query_missed())
